...
export function Tooltip({ children, text, content }) {
    ...
    if (content) {
        trigger = (
            <span className="cursor-pointer decoration-dotted underline underline-offset-4" {...triggerProps} {...hoverProps}>
                {content}
            </span>
        );
    } else if (isReactText(children)) {
        trigger = (
            <span className="cursor-pointer border-b border-dotted" {...triggerProps} {...hoverProps}>
        {children}
      </span>
        );
    } else {
