...
export default function Content(props) {
    ...
    const Component = React.useMemo(() => getMDXComponent(currentDoc.body.code), [currentDoc.body.code])

  return (
    <div className={"wrapper-content"}>
      <div className={"pb-4"}>
        <p className={"font-semibold text-primary capitalize"}>{CleanHyphen(currentSlug)}</p>
        <h1 className={"py-5 text-4xl font-semibold"}>{currentDoc.title}</h1>
          <Component components={render}/>
      </div>
      <Footer currentVersion={currentVersion} currentSection={currentSection} currentSlug={currentSlug} editBase={editBase} />
    </div>
  );
}
