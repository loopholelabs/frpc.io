...
BenchmarkService implementation is a simple `Echo{:.keyword}` service that does little to no processing and does
not block.
...
Our blocking operation for the following benchmark is a simple `time.Sleep{:go}` call that sleeps for exactly 50 Microseconds.
