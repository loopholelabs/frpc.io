...
lower than that of existing protocols like gRPC (performance comparisons available in our [gRPC Benchmarks](https://frisbee.sh/performance/grpc-benchmarks))
...
to implement `Encoding{:.keyword}` and `Decoding{:.keyword}` packages, which read and write directly from the `Packet.packet{:.keyword}`
...
All these optimizations - as well as Frisbee's architecture, make it feasible to use Frisbee (as well as <Tooltip text={"Frisbee RPC"} content={"fRPC"} />)
...
<Note content="In our benchmarks we've tested both approaches, though it should be noted that the single-goroutine approach is not as efficient as the multi-goroutine approach when the blocking time of the RPC handler is high." />
...
<Note content="It's important to note that while Frisbee and fRPC were designed to be used with TCP connections, there's no reason developers can't use other transports. As long as the transport fulfills the 'net.Conn' interface, it will work as expected with Frisbee and fRPC." />
