...
[Apache Thrift](https://thrift.apache.org/)) that uses the [Frisbee](/concepts/frisbee) messaging framework as its underlying transport mechanism.
...
RPC Framework that uses Frisbee under the hood. Moreover, FRPC is a **drop-in
replacement for GRPC** thanks to its generated interfaces matching GRPC's, and in
an apples-to-apples comparison it [outperforms existing RPC frameworks]() by a significant margin.
