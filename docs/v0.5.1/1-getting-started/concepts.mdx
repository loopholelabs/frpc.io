...
<Tooltip
  text={
    "Request/Reply systems where the request is sent to a remote service which then replies back to the caller"
  }
  content={"RPC Frameworks"}
...
serialization framework that recycles byte buffers and can serialize and deserialize data with almost no allocations. T
...
implementation of `Polyglot{:.keyword}`, as well as [polyglot-ts](https://github.com/loopholelabs/polyglot-ts) which is a
TypeScript (and Javascript) implementation of `Polyglot{:.keyword}`.
...
fRPC generates a `frisbee.HandlerTable{:.keyword}` that allows Frisbee to route incoming packets to the correct
handler functions. It's possible to override this table using the `frisbee.Server.SetHandlerTable(){:go}`
method (which is exposed in the generated `frpc.Server{:.keyword}` type).
