...
- [Go](https://golang.org) - FRPC works with `Go{:.keyword}` version 1.17 or later. For installation instructions see [Go's Getting Started Guide](https://golang.org/doc/install).
...
To install the `protoc-gen-frpc{:.keyword}` plugin, you'll first need to make sure that your `$GOBIN{:.keyword}` environment variable is set and available in
