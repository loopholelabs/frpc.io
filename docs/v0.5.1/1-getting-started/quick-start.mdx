...
the `protoc-gen-go-frpc{:.keyword}` <Tooltip text={"Plugins hook into the protoc compiler and allow for custom code generation"} content={"protoc plugin"}/>
which we will use to generate the server and client code.

## Prerequisites

- [Go](https://golang.org) - fRPC works with `Go{:.keyword}` version 1.18 or later. For installation instructions see [Go's Getting Started Guide](https://golang.org/doc/install).
...
To install the `protoc-gen-go-frpc{:.keyword}` plugin, you'll first need to make sure that your `$GOBIN{:.keyword}` environment variable is set and available in
...
To install the `protoc-gen-go-frpc{:.keyword}` plugin itself, you'll need to run the following command:

```bash
$ go install github.com/loopholelabs/frpc-go/protoc-gen-go-frpc@latest
```

This will install the `protoc-gen-go-frpc{:.keyword}` plugin into your `$GOBIN{:.keyword}` directory
...
Now that we have the prerequisites and the `protoc-gen-go-frpc{:.keyword}` plugin installed,
...
```proto {5-7} title="~/frpc/echo.proto"
syntax = "proto3";
...
by specifying the `--go-frpc_out{:.keyword}` flag, we're implicitly specifying that we want to use the `protoc-gen-go-frpc{:.keyword}` plugin.
...
These commands should have generated a new folder at `~/frpc/echo{:.keyword}`, which
...
```go title="~/frpc/echo/echo.frpc.go"
...
...
the server. We'll start by creating a new `server/main.go{:.keyword}` file in our `~/frpc{:.keyword}` directory:

```go title="~/frpc/server/server.go"
package main
...
```go {22-32} title="~/frpc/server/server.go"
package main
...
We're passing in `nil{:.keyword}` for both the `*tls.Config{:go}` and `logging{:go}` parameters in the generated `echo.NewServer(){:go}` function because
...
in our `~/frpc{:.keyword}` directory:

```go title="~/frpc/client/client.go"
package main
...
If you want to learn more about how **fRPC** works under the hood, you can check out our [fRPC Concepts](/getting-started/concepts) page as well.

Finally, if you'd like to learn how to use [Frisbee](/frisbee/overview) (the underlying transport mechanism for fRPC)
to implement your **own messaging protocol** that's fast and performant, you can check out our [Custom Protocols with Frisbee](/frisbee/custom-protocols) guide.
